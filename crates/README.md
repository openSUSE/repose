...
`tests/vectors/` (see `scripts/check-cli.sh`), not by any earlier
implementation — see "The Python Implementation Is Not a Reference" in
[`AGENTS.md`](../AGENTS.md).

## Packaging (Rust)

**Replace model:** a single package ships one binary, `repose`, under the
package and binary name repose has always used. There is no parallel-install
variant.
...
`repose` keeps its established package name (no `Provides` for a parallel
install). Man pages and completions are owned by the `repose-gen` helper and
guarded by the `rust-assets-drift` CI job.
